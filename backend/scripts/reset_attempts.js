...
async function checkAttempts() {
    try {
        ...
        console.log('Checking attempts for user ID 15 and part ID 4 (assignment 4)');
        ...
        if (assignment) {
            ...
            await db.execute('DELETE FROM submissions WHERE assignment_id = ? AND student_id = 15', [assignment.assignment_id]);
            console.log('Submissions deleted');

            // Delete attempts
            await db.execute('DELETE FROM assignment_attempts WHERE assignment_id = ? AND student_id = 15', [assignment.assignment_id]);
            console.log('Attempts deleted');

            // Delete results
            await db.execute('DELETE FROM assignment_results WHERE assignment_id = ? AND student_id = 15', [assignment.assignment_id]);
